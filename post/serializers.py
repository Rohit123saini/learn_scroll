...
#     class Meta:
#         model = PostSave
#         fields = ['id', 'post_id', 'user', 'username', 'collection_name', 'created_at']
#         read_only_fields = ['id', 'user', 'created_at']
#
#     def validate(self, attrs):
#         # post exist karta hai ya nahi check
#         if not Post.objects.filter(id=attrs['post_id'], is_deleted=False).exists():
...
class PostMediaSerializer(serializers.ModelSerializer):
    ...
    def validate_file(self, file):
        if file.size > 100 * 1024 * 1024: # 100MB fix kiya
            raise serializers.ValidationError("File size cannot exceed 100MB")
        return file
...
class PostListSerializer(serializers.ModelSerializer):
    ...
    def get_my_reaction(self, obj):
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            # FIX: tera related_name 'likes' hai, 'reactions' nahi
            like = obj.likes.filter(user=request.user).first()
            if like:
...
class PostSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Post
        fields = ['id', 'user', 'content', 'title', 'category', 'post_type', 'created_at', 'likes_count', 'comments_count', 'saves_count', 'like_count', 'confuse_count', 'wrong_count', 'imp_count', 'explain_count', 'my_reaction', 'is_saved', 'media']
...
# 🔥 FINAL FIXED PostSave Serializer
class PostSaveSerializer(serializers.ModelSerializer):
    post_id = serializers.UUIDField(write_only=True)
    post = PostListSerializer(read_only=True) # save ke baad pura post dikhega
    username = serializers.CharField(source='user.username', read_only=True)
    ...
    def create(self, validated_data):
        ...
        # get_or_create se duplicate error nahi ayega
        save_obj, created = PostSave.objects.get_or_create(
            post=post,
            user=request.user,
