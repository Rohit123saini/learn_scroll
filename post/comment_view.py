...
class CommentCreateAPIView(APIView):
    ...
    def post(self, request):
        ...
        if data_copy.get('parent_id') in ['', 'string', 'null']:
            data_copy['parent_id'] = None
        if data_copy.get('post_id') in ['', 'string', 'null']:
            data_copy['post_id'] = None
...
class CommentHideAPIView(APIView):
    ...
    @extend_schema(summary="Hide/Unhide Comment (Post Owner)", parameters=[OpenApiParameter(name='comment_id', type=OpenApiTypes.UUID, location=OpenApiParameter.PATH)], tags=['Comments'])
    @transaction.atomic
