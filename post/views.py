...
#                     'metadata': {
#                         'type': 'object',
#                         'description': 'Extra JSON data'
#                     },
#                     'location': {
#                         'type': 'object',
#                         'description': '{"city": "Meerut", "country": "India", "lat": 28.9, "lng": 77.7}'
#                     },
#                     'media_files': {
#                         'type': 'array',
#                         'items': {'type': 'string', 'format': 'binary'},
#                         'description': 'Multiple files: images, videos, PDF, DOCX'
#                     },
#                     'media_types': {
#                         'type': 'array',
#                         'items': {
...
#                             'enum': ['image', 'video', 'document', 'audio', 'gif']
#                         },
#                         'description': 'Must match order of media_files'
#                     }
#                 },
...
#                 'Image Post',
#                 value={
#                     'content': 'Check my new setup #tech #workspace',
#                     'category': 'tech',
#                     'post_type': 'image',
#                     'visibility': 'public',
#                     'hashtags': ['tech', 'workspace']
#                 },
#             ),
#         ],
#         responses={201: PostCreateSerializer},
...
#     """
#     GET /post/feed/
#     Personalized feed with fallback to trending
#     """
#     serializer_class = PostListSerializer
#     permission_classes = [IsAuthenticated]
#     pagination_class = HomeFeedPagination
...
#         ).values_list('following_id', flat=True)
#
#         # 2. Time window for relevance
#         seven_days_ago = timezone.now() - timedelta(days=7)
#
#         # 3. Base queryset - optimized
...
#                 return following_posts
#
#         # 5. Cold start - trending public posts
#         return base_qs.filter(
#             visibility='public',
#             created_at__gte=seven_days_ago
#         ).annotate(
...
#     @extend_schema(
#         parameters=[
#             OpenApiParameter(name='target_user_id', type=str, description='User UUID to fetch posts'),
#             OpenApiParameter(name='category', type=str, description='Filter by category'),
#             OpenApiParameter(name='post_type', type=str, description='Filter by type'),
#         ],
#         description='Get posts - own profile or other user with privacy checks'
#     )
#     def get(self, request, *args, **kwargs):
#         return super().get(request, *args, **kwargs)
#
...
#         ).exists()
#
#         # Private account + not following = nothing
#         if target_user.is_private and not is_following:
#             return Post.objects.none()
#
#         queryset = base_qs.filter(user=target_user)
...
#             queryset = queryset.filter(post_type=post_type)
#
#         # Following or public account = public + connections
#         if is_following:
#             return queryset.filter(
#                 Q(visibility='public') | Q(visibility='connections')
#             ).order_by('-created_at')
...
#             return Response(
#                 {"success": False, "message": "Post is private"},
...
#                 return Response(
#                     {"success": False, "message": "Only connections can view"},
#                     status=status.HTTP_403_FORBIDDEN
#                 )
#
#         # Track view
...
#     file_name = os.path.basename(file_path)
#
#     # Video/Audio range support - INLINE serve karo
#     range_header = request.META.get('HTTP_RANGE', '')
...
#             response['Content-Length'] = str(length)
#             response['Accept-Ranges'] = 'bytes'
#             response['Content-Disposition'] = f'inline; filename="{file_name}"'  # 👈 VIDEO INLINE
...
#     response['Accept-Ranges'] = 'bytes'
#
#     # DOC/DOCX/XLS/XLSX/PPT/PPTX/ZIP/RAR = Force Download
...
#     elif is_doc:
#         response['Content-Disposition'] = f'attachment; filename="{file_name}"'  # 👈 DOC DOWNLOAD
#         response['Content-Type'] = 'application/octet-stream'
#     elif content_type.startswith(('image/', 'video/', 'audio/')):
#         response['Content-Disposition'] = f'inline; filename="{file_name}"'  # 👈 VIDEO/IMAGE INLINE
#         response['Cache-Control'] = 'public, max-age=31536000'
#     else:
#         response['Content-Disposition'] = f'attachment; filename="{file_name}"'
#
...
# from.serializers import ReactionRequestSerializer
#
# class PostReactionAPIView(APIView):
#     permission_classes = [IsAuthenticated]
#
#     @swagger_auto_schema(
#         operation_summary="Toggle Post Reaction (Like/Unlike)",
#         operation_description="Same reaction dubara bhejo to unlike ho jayega, alag bhejo to change ho jayega",
#         request_body=ReactionRequestSerializer,
#         responses={
...
#                         "status": "liked",
#                         "reaction": "like",
#                         "counts": {"like": 10, "confuse": 2, "wrong": 0, "imp": 5, "explain": 1, "total": 18},
#                         "my_reaction": "like"
#                     }
#                 }
#             )
...
#                 # UNLIKE - count ghat jayega
#                 existing.delete()
#                 status_msg = "unliked"
#                 my_reaction = None
#             else:
#                 # CHANGE - purana -1, naya +1
#                 existing.reaction_type = reaction_type
#                 existing.save()
#                 status_msg = "changed"
#                 my_reaction = reaction_type
#         else:
#             # LIKE - count badh jayega
#             PostLike.objects.create(post=post, user=user, reaction_type=reaction_type)
#             status_msg = "liked"
#             my_reaction = reaction_type
#
...
#
#     @swagger_auto_schema(
#         operation_summary="Get Post Reaction Summary",
#         operation_description="Post pe kitne like, confuse, wrong hai aur maine kya reaction diya hai",
#         responses={200: "Reaction summary"}
#     )
...
#         })
#
# # Swagger bina login ke GET ke liye alag permission chahiye to ye use kar sakta hai
# class PostReactionPublicAPIView(PostReactionAPIView):
