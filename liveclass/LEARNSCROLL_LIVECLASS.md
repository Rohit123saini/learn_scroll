...
| File | Lines | Role |
|---|---|---|
| `models.py` | 3502 | All DB models (~45 models) + model-level business logic + cache-version signals |
| `serializers.py` | 1803 (corrected this pass — previously misstated as 1772) | DRF serializers — **68 classes** (re-counted this pass — see §1 note below the table for the 13 previously-uncatalogued ones, 12 confirmed earlier plus `StudentReportCardSerializer` newly caught this pass) |
| `views.py` | 6508 (confirmed — matches the previous ~6510 estimate) | DRF ViewSets/APIViews — all HTTP endpoints, permission logic, orchestration. **+TASK 4** (new): `ClassroomViewSet.perform_create` now enqueues `notify_followers_new_classroom` via `_safe_delay` — see §4/§9. |
| `urls.py` | 510 (up from the previously-documented ~429 — corrected this pass; no missing-route regression found, see below) | DRF router registrations + a few plain `path()`s for non-ViewSet views, incl. the two new `create_group/`/`group/` paths (tasks 29/30, §5/§6b). **Its module docstring is itself the canonical endpoint reference** — reproduced in full in §5 below. |
...
| `test_classroom_chat_bridge.py` | 395 (corrected this pass, previously off by one at 396) / ~267 (documented test suite, unseen) | **NEW (task 40)** — *should be* regression tests for `core/classroom_chat_bridge.py` (create/accept/kick/refund/metadata-sync/archive), its own `test_*.py` module (Django auto-discovers `test*.py` per app), reusing `LiveClassTestBase` + the same `LIVEKIT_PATCH`/`SAFE_DELAY_PATCH` convention as `tests.py`. **🚨 CONTENT MISMATCH — confirmed again on this (fourth) upload, unchanged from before**: the file at this path is still `core/classroom_chat_bridge.py` **source code**, not test classes. Same failure shape as the historical `notification_batching.py`/`test_notification_batching.py` mix-up. Upside of this repeat mismatch: the bridge source itself is now confirmed (used to write the row above). **The actual test suite for this module still has not been seen** — §16's description of its 6 test classes remains last-confirmed history, not verified-current, across four uploads now. |
...
**13 previously-uncatalogued output/action serializers confirmed in `serializers.py`** (mostly `serializers.Serializer`, not `ModelSerializer` — plain response shapers for endpoints §5 already documents by URL, just not previously matched to a class name here): `ClassroomStatsSerializer`, `SessionRecordingSerializer`, `StudentProgressSerializer` (backs `my-progress/`), `TeacherEarningsSerializer` + `EarningsByDaySerializer` + `EarningsByClassroomSerializer` (back `my-earnings/`), `ReferralSerializer` + `MyReferralCodeSerializer` + `ReferralRedeemSerializer` (referral flow), `ClassroomBanSerializer`, `ClassroomMyShareSerializer` + `ClassroomShareLogSerializer` (share history), and **`StudentReportCardSerializer`** (`ModelSerializer`, newly caught this pass — backs `ReportCardViewSet` in `parent_link_views.py`, §6c). None of these change any documented endpoint's behavior or shape claims elsewhere in this file — flagged here purely so the class count doesn't get copied forward stale again. `ParentJoinSerializer` was already tracked (see §6c — it's dead code left over from the pre-consolidation parent-join flow, not one of the 13).
...
- **`ReportCardViewSet`** (manage-tier for create/update) — `attendance_percent` is **never** accepted
  ...
  - 🐛 **Two confirmed bugs in `_homework_stats()`, both currently make report-card creation crash or
    lie**: (1) it filters `assigments.objects.filter(classroom=classroom, category=assigments.Category.
    HOMEWORK)` — but `assigments` (§3) has **no `category` field and no `Category` enum at all** (see the
    ...
    fixed, this method reads the **local, legacy `liveclass.assigments`/`assigmentsSubmission` models
    directly** (§3) — but per Task 12 (§6d), new assigmentss no longer get created there at all; they go
    through the unified `assigments` app via `bridge.create_assigments()`. Left as-is, a report card's
    homework numbers would be correct only for assigmentss created *before* the Task 12 cutover and
    silently blind to every assigments posted after it — a report card that looks complete but is
    ...
    fixes this should route `_homework_stats()` through `bridge.get_assigments_submissions(classroom)`
    (§6d) instead, the same source `assigmentsViewSet`/`assigmentsSubmissionViewSet` (`views.py`) already
    use post-Task-12.
...
**Sibling test module**: `test_classroom_chat_bridge.py` (§1, §6b) is a **separate** file, discovered automatically alongside `tests.py` by Django's `test*.py` convention — `python manage.py test liveclass` runs both without extra config. **RE-CONFIRMED STILL MISMATCHED on this (fourth) upload**: the file at this path is still `core/classroom_chat_bridge.py` **source code**, byte-for-byte the same kind of mismatch as the last three uploads — not the 6 test classes (`CreateClassroomGroupTests`, `JoinAcceptChatSyncTests`, `RemovalChatSyncTests`, `MetadataSyncTests`, `ArchiveOnCloseTests`, `ClassroomStaffChatSyncTests`) this doc has been describing from memory since. Four uploads running under this filename, four copies of the bridge source, zero sightings of the actual test file — at this point treat "the test suite for this module has 6 classes covering X/Y/Z" as **unverified legacy description, not current fact**, until a genuinely different file shows up under this name.
...
30. ✅ **(PRODUCTION FIX PASS — the three item-24 bugs and the `ParentMessageTemplate`/`ParentTeacherMessage` admin gap actually fixed in code this pass, not just re-confirmed.)** Scope: `urls.py`, `views.py`, `parent_link_views.py`, `admin.py`.
    ...
    - **`parent_link_views.py`**: `ReportCardViewSet._homework_stats()`'s crash-causing `category=assigments.Category.HOMEWORK` filter is removed (every classroom assigments is now counted — there was never a homework/other distinction on this model). **Not fully closed**: still reads the local, Task-12-frozen `assigments`/`assigmentsSubmission` models directly rather than `bridge.get_assigments_submissions()`, so post-cutover assigmentss still won't show up here — left as an explicit in-code flag rather than guessed at, since `assigments/bridge.py`'s row shape still hasn't been part of any upload. See §6c, item 24.
    ...
    - **Not touched, no source available to fix safely**: the `assigments`-bridge migration gap flagged above, the unconfirmed `Notification.NotifType.CLASSROOM_CREATED_BY_FOLLOWED` reference (item 26), and anything living in `core/classroom_chat_bridge.py`/`settings.py`/`exceptions.py`/`apps.py`/`consumers.py` — none of these were part of this upload.
