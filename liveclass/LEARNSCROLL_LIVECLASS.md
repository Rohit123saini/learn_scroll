...
assigmentss, breakout rooms, etc. Coins are released to the teacher **per
...
> **Cross-app moves to know about before touching money, assigmentss, or test series:**
...
> classroom **assigmentss** now route through the unified, project-wide `assigments`
> app via `liveclass/bridge.py` (`liveclass.assigments`/`assigmentsSubmission` are
...
> **New this pass**: `liveclass/bridge.py` also gained `create_testseries()` — a live
> creation path into the unified `testseries` app (no local `TestSeries`-equivalent
> model existed in `liveclass` to migrate away from, so this is a new door, not a
> backfill like the other two) — see §6d.
...
| File | Lines | Role |
|---|---|---|
| `models.py` | 3539 (up from 3502 this pass — `StudentReportCard` gained a real `created_by` FK, migration `0003_studentreportcard_created_by.py`; see §3) | All DB models (~45 models, 44 confirmed by class count) + model-level business logic + cache-version signals |
| `serializers.py` | 1759 (down from 1803 this pass — `ParentJoinSerializer` deleted, TASK 20) | DRF serializers — **67 classes** (down from 68 — see §1 note below the table) |
| `views.py` | 6498 (down from 6508 this pass — `PARENT_JOIN_TOKEN_SALT`/`generate_parent_join_token()` deleted, TASK 20, see §6c) | DRF ViewSets/APIViews — all HTTP endpoints, permission logic, orchestration. **+TASK 4** (new): `ClassroomViewSet.perform_create` now enqueues `notify_followers_new_classroom` via `_safe_delay` — see §4/§9. |
| `urls.py` | 525 (up from 510 this pass — comment growth from the TASK 15 path-converter fix, no new routes) | DRF router registrations + a few plain `path()`s for non-ViewSet views, incl. the two new `create_group/`/`group/` paths (tasks 29/30, §5/§6b). **Its module docstring is itself the canonical endpoint reference** — reproduced in full in §5 below. |
| `admin.py` | 606 (corrected this pass — previously off by one at 607) | Django admin registrations (inlines, list filters, bulk actions) — 33 `ModelAdmin`s for the app's original business models, **plus `ParentMessageTemplateAdmin`/`ParentTeacherMessageAdmin` added in the production-fix pass (§17 item 30) — 35 total**, plus the conditional `_AutoUserAdmin` fallback (not counted in that figure — see §14); not 40+ — `NotificationAdmin`/`NotificationPreferenceAdmin` moved to `core/admin.py` per task 42, since `Notification` itself now lives in `core/models.py`; see §14. **Gap now closed (§17 item 30)**: `ParentMessageTemplate`/`ParentTeacherMessage` are registered with real `ModelAdmin`s. |
...
| `consumers.py` | 577 (confirmed exact this pass) | Django Channels WebSocket consumers (`SessionConsumer`, `UserConsumer`, `ClassroomConsumer` — new, see §6) |
| `routing.py` | 30 | WebSocket URL patterns (separate from `urls.py`, wired into ASGI, not WSGI) — 3 routes: session/user/classroom |
| `liveclass/ws_auth.py` | 53 (re-confirmed, exact match) | JWT-over-WebSocket auth middleware — **thin re-export only** (`from LearnScroll.ws_auth import JWTAuthMiddleware, get_user_from_token`), kept so every existing `from liveclass.ws_auth import JWTAuthMiddleware` call (including `asgi.py`) keeps working unchanged after the consolidation below. |
...
| `realtime.py` | 558 | Low-level pub/sub helpers: Redis-backed broadcast (session/user/classroom), presence, missed-event history, connect rate-limit |
| `livekit_utils.py` | 472 (up from ~434 — growth is added explanatory `NOTE (fix)` comments, no new public function; still exactly `LiveKitError`, `ParticipantRole`, `_grants_for_role`, `generate_livekit_token`, `_client`, `verify_webhook_event`) | LiveKit token generation, room API client, webhook signature verification |
| `moderation.py` | 137 | Lightweight rule-based chat moderation (`screen_message`) — profanity/spam/caps detector |
| `notifications.py` | 260 (confirmed exact this pass) | Single fan-out point for push/email/SMS/WhatsApp — provider-agnostic |
| `exceptions.py` | 184 | DRF custom exception handler — normalises every error response to one JSON shape |
| `chunked_upload_views.py` | 607 (confirmed exact this pass) | Chunked file upload (init/chunk/complete/abort) for large files (cover images, recordings, materials) |
| `bridge.py` | 267 (up from 141 — a third function was added this pass) | **NEW (Task 12), now also `liveclass`'s door into the unified `testseries` app** — `create_assigments()` / `get_assigments_submissions()` (mirrors `campus/bridge.py`'s Task 11 pair field-for-field) **plus new `create_testseries()`** (testseries_app_reference.md §3.6, was flagged "still open" — mirrors `campus.bridge.create_testseries()`'s described pattern). Golden rule: `assigments`/`testseries` never import `liveclass.*`, and `liveclass` never imports `assigments.models.assigments`/`assigmentsSubmission`/`testseries.models.TestSeries` directly anywhere outside this module (or the one-off backfill command below). See §6d. |
| `management/commands/migrate_liveclass_assigmentss_to_unified.py` | 258 | **NEW (Task 12)** — one-off, idempotent, rollback-logged data migration: copies every legacy `liveclass.assigments`/`assigmentsSubmission` row into the unified `assigments` app so history survives the cutover to `bridge.py`. See §6d. |
...
| `classroom_chat_views.py` | 86 (re-measured this pass; previously noted as 95 — same 2 views, no functional change) | **NEW (tasks 29/30)** — `ClassroomCreateGroupView` (teacher-only "haan" confirm, POST) + `ClassroomGroupStatusView` (manager-tier read) for the classroom↔chat-group bridge. Deliberately its own file/own explicit `path()`s rather than `ClassroomViewSet` actions — see §6b. **✅ (Task 3) manager-tier check now reuses the real `_can_manage_classroom()` from `views.py`** instead of a local hand-rolled duplicate — see §6b known-gaps. |
| `core/classroom_chat_bridge.py` | 677 (up from 396 — real growth, not re-measurement noise) | **NEW, outside this app** (lives in the `core` app). ✅ **Source now directly uploaded and confirmed** (own filename this time, not inferred via the `test_classroom_chat_bridge.py` mismatch). **12 public entry points now, not 9** — the module gained a `campus`-facing pair this pass and its own docstring explicitly renumbered `resolve_parent_from_token()` from #9 to #12 to keep the new pair contiguous (no behavior change, just docstring reorg): `create_classroom_group(classroom, actor)` (idempotent, teacher-only, raises `ValueError` for a non-teacher), `get_groups_for_classrooms(classrooms)` (bulk read-only lookup, `{classroom_id: Group}`), `sync_membership_on_join_accept(classroom, student)`, `sync_membership_on_removal(classroom, student, reason="")`, `promote_to_moderator(classroom, user)`, `sync_group_metadata(classroom)`, `archive_group_on_classroom_close(classroom)`, `post_welcome_message(classroom)`, `post_session_live_announcement(session)`, **`create_section_group(section, actor)`** and **`provision_video_room(live_session, actor)`** (both NEW — `campus.Section`/`campus.CampusLiveSession` counterparts, called from `campus/bridge.py`, not from anything in this `liveclass` app), and `resolve_parent_from_token(token)`. The two new campus functions are **not relevant to `liveclass`** itself (no call site in this app's own files) — noted here only because they live in the same shared module `liveclass` also depends on. Referenced by `classroom_chat_views.py`, `signals.py` (6 call sites — see §13), `tasks.notify_session_live` (`post_session_live_announcement`), and (per its own docstring) `liveclass/views.py`. Every function except `create_classroom_group`/`create_section_group` is a silent no-op if the classroom/section has no linked group. The `⚠️ ASSUMPTION` markers this row previously flagged about `ClassJoinRequest`/`ClassroomStaff` field shapes are gone from the current source's own docstring — it now states those were verified against the real `liveclass/models.py`, matching what this doc already independently confirmed (§3). |
| `test_classroom_chat_bridge.py` | 492 (up from 395 — see below, this is now genuinely different content) | **NEW (task 40)** — regression tests for `core/classroom_chat_bridge.py`. **✅ RESOLVED this pass, on the fifth upload under this filename**: the content mismatch that persisted across four prior uploads (this path repeatedly containing `core/classroom_chat_bridge.py`'s own source instead of tests) is fixed — this is now a real, if labelled TASK 29, `django.test.TestCase`-based test module with **11 test classes** covering `_get_group_for_classroom`/`_classroom_cover_image_url` (the two internal helpers) plus 9 of the module's public functions (everything except the two campus-only additions and `resolve_parent_from_token`, which this app has no reason to test here). Uses `SimpleNamespace`-based `FakeClassroom`/`FakeSession` stand-ins and mocks every cross-app import target, rather than real `liveclass`/`message` model instances — its own docstring flags this as a deliberate placeholder pending a real-model upgrade. See §16. |
| `throttles.py` | 41 (line count confirmed this pass) | **NEW (task 9)** — app-specific `SimpleRateThrottle` subclasses that don't fit the everyday `ScopedRateThrottle` (user-keyed) pattern. Currently one class: `ParentJoinIPThrottle` (scope `session_parent_join_ip`) — see §6c. Same single-purpose-throttles-file convention `message/throttles.py` already uses. |
...
| `parent_link_views.py` | 360 (up from 318 this pass — Task 5 fix, see below) | **NEW FILE, ✅ now wired (Task 10 fix)** — teacher-managed `ParentAccessCode` endpoints: Phase 2 (`ClassroomParentCodeGenerateView` — teacher generates a code for a student), Phase 4 (`ReportCardViewSet` — server-computed report cards), Phase 5 teacher-side (`ClassroomParentQueryListView` + `ParentQueryReplyView` — parent-mode query threads). Cross-app: imports `ParentAccessCode`/`ParentModeQuery`/`ParentModeQueryMessage` from `message.models` and `create_bell_rows_for_push`/`send_parent_push` from `message.services`/`message.push_utils` — the same "liveclass calls into message, never the reverse" direction as `core/classroom_chat_bridge.py`. All four views now have real `path()`/`router.register()` entries in `urls.py`, and the `StudentReportCard` model they depend on is now defined in `models.py` — see §6c for the full picture (including the parent_join() consolidation onto this same `ParentAccessCode` mechanism). **✅ (Task 5 fix, this pass)**: `ReportCardViewSet.create()` now stamps `created_by=request.user` on every report card, and `_homework_stats()` now sources homework numbers via `bridge.get_assigments_submissions()` (the unified `assigments` app) instead of reading the legacy, Task-12-frozen local tables directly — closes the "still reads stale local models" gap items 29/30 had left open. See §6c/§6d and §17 item 31. |
| `tests.py` | 2885 (up from ~2610) | Test suite (Django `TestCase`), **26** test classes organized by feature area (re-counted this pass — one more than previously documented: `ParentTeacherMessageTests`, regression coverage for the pre-existing `ParentMessageTemplate`/`ParentTeacherMessage` student→teacher template-message feature, not the newer `ParentAccessCode`-based §6c flow) — see §16 for the confirmed gap (still no `StudentProgressTests`) |
| `settings.py` | ~890 | **Project-level** Django settings (not liveclass-only — shared with `login`/`message` apps) |

**13 previously-uncatalogued output/action serializers confirmed in `serializers.py`** (mostly `serializers.Serializer`, not `ModelSerializer` — plain response shapers for endpoints §5 already documents by URL, just not previously matched to a class name here): `ClassroomStatsSerializer`, `SessionRecordingSerializer`, `StudentProgressSerializer` (backs `my-progress/`), `TeacherEarningsSerializer` + `EarningsByDaySerializer` + `EarningsByClassroomSerializer` (back `my-earnings/`), `ReferralSerializer` + `MyReferralCodeSerializer` + `ReferralRedeemSerializer` (referral flow), `ClassroomBanSerializer`, `ClassroomMyShareSerializer` + `ClassroomShareLogSerializer` (share history), and **`StudentReportCardSerializer`** (`ModelSerializer` — backs `ReportCardViewSet` in `parent_link_views.py`, §6c). None of these change any documented endpoint's behavior or shape claims elsewhere in this file — flagged here purely so the class count doesn't get copied forward stale again. **✅ `ParentJoinSerializer` is now gone (TASK 20, this pass)** — see below; `serializers.py` is now **67** classes / **1759** lines (down from 68/1803), the class count dropping by exactly the one deleted class.
...
User (login.User, AbstractUser — no fixed teacher/student role)
 └─ creates Classroom (becomes teacher for THIS classroom only)
     ...
     ├─ ClassMaterial, assigments → assigmentsSubmission → grade
...
- **`chat_group_enabled`** (bool, default False), **`linked_conversation_id`** (UUID, nullable) — ✅ **defined on `Classroom` and migrated** (§1, §6b — landed as a `🔧 GAP FIX (Gap 2 prerequisite)` right in the model; no pending manual migration step). Gates whether any of `core/classroom_chat_bridge.py`'s sync functions do anything at all; `linked_conversation_id` points at the `message` app's `Conversation` backing the linked `Group` (plain `UUIDField`, not a FK — keeps `liveclass` decoupled from `message`'s models).
...
### `assigments`, `assigmentsSubmission` — ⚠️ **local models frozen/legacy as of Task 12**
`assigmentsSubmission.is_late()`. Schema unchanged (no migration), but as of Task 12 these two models are **no longer the live write/read path for new assigmentss** — `liveclass/bridge.py`'s `create_assigments()`/`get_assigments_submissions()` routes new classroom assigmentss through the project-wide unified `assigments` app instead (see §6d). These rows still exist for **historical data only**, backfilled into the unified app one-time via `migrate_liveclass_assigmentss_to_unified` (§6d) — `assigmentsAdmin`/`assigmentsSubmissionAdmin` (§14) stay registered so that history stays visible in Django admin, but no new code path should create rows here going forward.
...
**✅ NEW this pass — `created_by` field added** (migration `0003_studentreportcard_created_by.py`,
`liveclass.0002_initial` → `0003`). `ForeignKey(User, on_delete=SET_NULL, null=True, blank=True,
related_name="report_cards_created")` — nullable/`SET_NULL` rather than the required-`CASCADE` shape
`LivePoll`/`Coupon`/`ClassHoliday`'s own `created_by` use, specifically because this table already had
rows with no way to know after the fact who created them (a fake backfill default was rejected as worse
than null — see the field's in-code comment). `StudentReportCardSerializer` had already listed
`created_by` in `fields`/`read_only_fields` before the column existed, which is what was crashing
drf-spectacular's schema generation (`ImproperlyConfigured: Field name 'created_by' is not valid for
model 'StudentReportCard'`) — not a serializer bug, the model was just missing the column. Now set on
every new row: `ReportCardViewSet.create()` (`parent_link_views.py`) passes `created_by=request.user`
into `update_or_create()`'s `defaults`, same pattern `ClassHolidayViewSet.perform_create()` already uses
for `ClassHoliday.created_by`. **Gap, not previously flagged**: `StudentReportCard` still has no
`ModelAdmin` registered in `admin.py` (not one of the 35 listed in §14) — no drift, just newly noticed
while re-checking `models.py`/`admin.py` together this pass.
...
| `assigmentsViewSet` (4704) | ⚠️ **no longer accurate — see §6d** | — |
| `assigmentsSubmissionViewSet` (4815) | ⚠️ **no longer accurate — see §6d** | — |
...
| `my-progress/` | GET | `StudentProgressView` — caller's own attendance/assigments/certificate stats + streak |
...
assigmentss/                         GET, POST         ⚠️ STALE per this urls.py docstring — see §6d/§17.
                                                          As of Task 12, assigmentsViewSet is a plain
                                                          viewsets.ViewSet with ONLY list/create; no
                                                          retrieve/update/destroy exist any more (both
                                                          proxy to bridge.py -> the unified `assigments`
                                                          app). GET filters by required ?classroom=.
assigmentss/{id}/                    ❌ REMOVED (Task 12) — retrieve/update/destroy are gone; the unified
                                                          assigments.views.assigmentsViewSet doesn't serve
                                                          context-sourced (campus/liveclass) assigmentss
                                                          either, so there is currently NO way to edit or
                                                          delete an already-posted classroom assigments
...
submissions/                         GET only          ⚠️ STALE per this urls.py docstring — POST/grade
                                                          ...
                                                          optional ?assigments=) or a student's own rows.
...
submissions/{id}/grade/              ❌ REMOVED (Task 12) — grading now happens directly against the
                                                          ...
                                                          (`/assigments/submissions/{id}/...`, not part of
                                                          this app). See §6d.
...
- `test_classroom_chat_bridge.py` (§1, §16) — `core/classroom_chat_bridge.py`'s own regression suite. **✅ Real content confirmed this pass** (fifth upload under this filename, first correct one): 11 `TestCase` classes — `GetGroupForClassroomTests`, `GetGroupsForClassroomsTests`, `ClassroomCoverImageUrlTests`, `CreateClassroomGroupTests`, `SyncMembershipOnJoinAcceptTests`, `SyncMembershipOnRemovalTests`, `PromoteToModeratorTests`, `SyncGroupMetadataTests`, `ArchiveGroupOnClassroomCloseTests`, `PostWelcomeMessageTests`, `PostSessionLiveAnnouncementTests` — not the 6-class shape this doc had been carrying forward from memory. Built on `SimpleNamespace`-based fakes + mocked cross-app imports rather than real model instances (its own docstring flags this as provisional, pending `liveclass.models`/`message.models` being wired in for a real integration-test upgrade).
...
- **`ReportCardViewSet`** (manage-tier for create/update) — `attendance_percent` is **never** accepted
  ...
  model — see §1/§5 (Task 10 fix). Every write also now stamps `created_by=request.user` (§3, new field
  this pass).
  - ✅ **Both formerly-open `_homework_stats()` bugs are now fixed (§17 items 30 & 31)**: (1) the crash —
    it used to filter `assigments.objects.filter(classroom=classroom, category=assigments.Category.
    HOMEWORK)`, but `assigments` (§3) has no `category` field/enum at all, so this raised `AttributeError`
    on the very first `POST report-cards/` for any classroom — the filter is removed, every classroom
    assigments now counts (there was never a homework/other distinction to filter by). (2) the staleness —
    it used to read the **local, legacy `liveclass.assigments`/`assigmentsSubmission` models directly**,
    which stop receiving new rows after the Task 12 cutover (§6d) — it now routes through `bridge.
    get_assigments_submissions(classroom)` instead, the same unified-app source `assigmentsViewSet`/
    `assigmentsSubmissionViewSet` (`views.py`) already use post-Task-12, so post-cutover assigmentss are
    counted correctly.
...
### Dead code left behind by the merge — ✅ now deleted (TASK 20, this pass)

`parent_join()` no longer needed a bespoke signed token, and three pieces that were flagged unused in
earlier passes have now actually been removed from the codebase:

- `PARENT_JOIN_TOKEN_SALT` / `generate_parent_join_token(student)` (`views.py`) — the old
  `django.core.signing`-based token minter. **Deleted.**
- `ParentJoinSerializer` (`serializers.py`, §20b) — the old validator for that signed token. **Deleted.**

Both deletion sites leave an explanatory comment block in place of the removed code, citing a confirmed
project-wide grep for `generate_parent_join_token`/`PARENT_JOIN_TOKEN_SALT`/`ParentJoinSerializer` that
found no remaining callers/importers beyond their own (now-removed) definitions. `views.py` shrank
6508→6498 lines and `serializers.py` shrank 1803→1759 lines (68→67 classes) as a result — see §1.
...
## 6d. `liveclass` ↔ `assigments` bridge (Task 12) — **NEW**

Same shape as §6b/§6c: a project-wide feature (here, a **unified cross-app `assigments` app**, shared
with at least the `campus` app — Task 11's `campus/bridge.py` is its sibling/reference implementation)
gets exactly one door into `liveclass`, and `liveclass`'s own local `assigments`/`assigmentsSubmission`
models (§3) step back to read-only history.

**The rule (same golden rule `campus/bridge.py` and `assigments/models.py`'s own docstring both state):**
`assigments` never imports `liveclass.*`. `liveclass` never imports `assigments.models.assigments` /
`assigmentsSubmission` directly anywhere **outside `bridge.py`** — with one deliberate, audited, one-off
exception: `migrate_liveclass_assigmentss_to_unified` (below) talks to `assigments.models` directly,
...
### `bridge.py` — now three functions (a `testseries` bridge joined the two `assigments` ones this pass)

- **`create_assigments(*, classroom, posted_by, title, description="", attachment=None, due_date=None)`**
  — delegates to `assigments.bridge.create_context_assigments(source=assigmentsSource.LIVECLASS,
  ...
  `Classroom` — `assigments` stores it opaquely.
  - **Roster source**: `PassPurchase(status=SUCCESS, is_active=True, expires_at__gt=now)` against the
    classroom's passes — the **exact same filter** `assigmentsViewSet.perform_create` (old
    `liveclass/views.py`) already used to fan out `assigments_POSTED` notifications, not a new query.
    `SessionParticipant` was considered and **rejected** as the roster source: it's per-`ClassSession`
    attendance, but an `assigments` here is classroom-scoped (no per-session `context_type` on the
    unified model — it's exactly `"section" | "classroom" | ""`), so session-level would be the wrong
    grain.
  - **Paid/unpaid is never asked, structurally** — the unified `assigments.models.assigments` has no
  ...
  - **`due_date` gotcha**: the unified `assigments.due_date` is a `DateField`; the old local
    `liveclass.assigments.due_date` was a `DateTimeField`. `create_assigments()` does **not** silently
    `.date()` a datetime passed in — a caller passing a full `datetime` gets whatever `assigments`'s own
  ...
  - **⚠️ Open assumption, flagged not guessed around**: `assigments.bridge.create_context_assigments()`'s
    ...
    `create_context_assigments()` treats a missing key the same way `campus`'s own docs already note for
    a blank `enrollment_no` (defaults to `""`, not `KeyError`). **Needed to close this**:
    `assigments/bridge.py` itself (not part of this upload — verify before this ships).
- **`get_assigments_submissions(classroom)`** — delegates to `assigments.bridge.
  get_submissions_for_context(context_type="classroom", context_id=classroom.id)`. **Unfiltered by
  permission**, same contract `campus.bridge.get_assigments_submissions()` documents — the caller
  (`views.py`) owns any further teacher/student-scoped narrowing (mirror the old
  `assigmentsSubmissionViewSet.get_queryset`'s `_can_manage_classroom` vs. `student=user` split).
- **`create_testseries(*, classroom, creator, title, description="", is_paid=False, price_coins=0,
  duration_minutes=None, attempts_allowed=1, questions)`** — **NEW this pass**, closes the
  testseries_app_reference.md §3.6 item that was previously flagged "still open". Delegates to
  `testseries.bridge.create_context_testseries(source=TestSeries.Source.LIVECLASS, context_type="classroom",
  context_id=classroom.id, ...)` — same opaque `(context_type, context_id)` pointer shape
  `create_assigments()` above already establishes; `testseries` never resolves it back to a real
  `Classroom` itself.
  - **No `subject` parameter** — same reasoning `campus.bridge.create_testseries()` documents for
    itself: `create_context_testseries()` has no subject-shaped slot at all, and `liveclass` has no
    `Subject` model to pull one from anyway.
  - **`is_paid`/`price_coins` passed straight through, no force-reset** — unlike
    `campus.bridge.create_testseries()`, there is no `testseries_paid_allowed`-equivalent flag anywhere
    in `liveclass` to gate these against (confirmed absent, not just unwired). A per-classroom/per-teacher
    paid/unpaid gate the way campus has one at the `Campus` level would need a new field + a new check
    here — this function can't infer that today.
  - **`questions`** — passed through uninspected to `create_context_testseries()`; validation is entirely
    `testseries.Question.full_clean()`'s own contract.
  - **Roster is `PassPurchase(status=SUCCESS, is_active=True, expires_at__gt=now)`** — same source
    `create_assigments()` above uses — **but the shape is different from that function's roster**:
    `create_context_testseries()`'s own `roster` param wants an iterable of real `login.User` instances
    (used only to fan out the `TESTSERIES_POSTED` notification — `testseries` fires it itself once handed
    the roster), not `create_assigments()`'s `{"user_id": ...}` dicts. This function resolves actual
    `User` rows (`User.objects.filter(id__in=student_ids)`) for exactly that reason — genuinely two
    different roster shapes across the two sibling bridge functions in this same file, not a typo in
    either one.
  - **⚠️ Needed to fully close this**: `testseries/bridge.py` itself (not part of this upload — the
    function above is written against `campus/bridge.py`'s *described* pattern in
    `campus_app_design.md`, since the real `campus/bridge.py` source wasn't part of this pass either).

### `migrate_liveclass_assigmentss_to_unified` (management command)

One-off **data** migration (no `makemigrations` involved) — backfills every historical
`liveclass.assigments`/`assigmentsSubmission` row into the unified `assigments` app so old assigmentss
survive the cutover. Deliberately does **not** reuse `bridge.create_assigments()` (that always
...
- **Roster for a backfilled assigments** = whoever actually submitted historically **UNION** whoever
  ...
  `MISSING` placeholder (matching what `bridge.create_assigments()` would produce for them going
  forward).
- **Idempotent**: every migrated `assigments` is stamped with `data["legacy_liveclass_assigments_id"] =
  str(old.id)`; re-running skips any old assigments whose new row already has that stamp (a Postgres
  `jsonb` lookup, `data__legacy_liveclass_assigments_id` — matches the `GinIndex`-on-Postgres assumption
...
- **Rollback**: every successful migration appends one JSON line (`old_assigments_id`,
  `new_assigments_id`, `old_submission_ids`, `new_submission_ids`) to `--log-file` (default
  `liveclass_assigments_migration_log.jsonl`); `--rollback <path>` reads it back and deletes exactly
  those new rows (submissions first, then assigmentss — FK order).
- **Usage**: `python manage.py migrate_liveclass_assigmentss_to_unified [--dry-run]
...
full assigmentss/submissions CRUD as if they were live, ordinary endpoints. Per this audit's direct read
of `views.py`, all of the coin ones are 410 Gone and most of the assigments ones don't exist at all (§5,
...
- `_validate_and_build_extra_data(purpose, data, user)` (207) — purpose-specific validation. `purpose` drives what `extra_data` JSON must contain: `material` needs `classroom_id`/`material_type`/optional `session_id`; `assigments_attachment`/`submission_file` need `assigments_id`.
...
- **Registered `ModelAdmin`s** (one per model, list filters/search/bulk actions tuned per model): `ClassroomAdmin`, `ClassScheduleAdmin`, `ClassSessionAdmin`, `ClassPassAdmin`, `PassPurchaseAdmin`, `PassGiftAdmin`, `PassDailyChargeAdmin`, `ClassJoinRequestAdmin`, `BreakoutRoomAdmin`, `SessionParticipantAdmin`, `ClassMaterialAdmin`, `ChatMessageAdmin`, `ChatMessageReportAdmin`, `LivePollAdmin`, `PollResponseAdmin`, `assigmentsAdmin`, `assigmentsSubmissionAdmin`, `ClassroomReviewAdmin`, `ClassroomWishlistAdmin`, `CouponAdmin`, `CoinTransactionAdmin`, `CoinPurchaseAdmin`, `CoinWithdrawalAdmin`, `ClassroomStaffAdmin`, `SessionWaitlistAdmin`, `ClassroomReportAdmin`, `CertificateAdmin`, `ClassReminderAdmin`, `ClassHolidayAdmin`, `NoticeAdmin`, `ClassQueryAdmin`, `ReferralAdmin`, `ClassroomBanAdmin`. That's 33, **not** the 40+ this file previously claimed — see the correction below. **Plus, added in the production-fix pass (§17 item 30)**: `ParentMessageTemplateAdmin`, `ParentTeacherMessageAdmin` — 35 total.
...
- **✅ RESOLVED (§17 item 30) — was: imported but never registered**: `ParentMessageTemplate` and `ParentTeacherMessage` (task 69's structured parent/student→teacher messaging, §5 `parent-message-templates/`/`parent-messages/`) are both imported at the top of this file from `.models`, and now each has a real `@admin.register(...)`/`ModelAdmin`: `ParentMessageTemplateAdmin` is editable (category/title/body_template/requires_session/is_active/display_order — platform-curated copy meant to be staff-tunable without a deploy, `list_editable` on `display_order`/`is_active`), `ParentTeacherMessageAdmin` is read-mostly (`resolved_message`/`created_at` in `readonly_fields` — `resolved_message` is an audit-trail snapshot taken at send time and must never be hand-edited, same reasoning `ReferralAdmin` already uses for its own financial-record fields). Same shape of fix as the historical `PassGift`/`ChatMessageReport`/`Referral`/`ClassroomBan` gaps already called out in this file's own comments.
- ⚠️ **NEW GAP, flagged this pass**: `StudentReportCard` (§3) has **no `ModelAdmin`** registered anywhere in `admin.py` — it's not among the 35 above, and no `@admin.register(StudentReportCard)` exists in the file. Not a regression (nothing removed it), just never had one to begin with; only surfaced now while cross-checking `models.py`'s new `created_by` field against `admin.py`. Debugging a report card currently means going through the API/ORM shell, not Django admin.
...
## 16. Testing (`tests.py`, 2885 lines, 26 test classes — confirmed by direct count this pass)
...
| `ParentTeacherMessageTests` | **NEW this pass, previously uncatalogued.** `ParentMessageTemplateViewSet` (the template picker — active-only listing, `?category=` filter, never exposes the raw `{child_name}`/`{classroom_title}` placeholder body_template to the sender) and the send flow for the pre-existing `ParentMessageTemplate`/`ParentTeacherMessage` feature. This is the **older**, template-driven student→teacher message feature — a distinct thing from both `parent_link_views.py`'s `ParentAccessCode`-based Phase 5 query threads (§6c) and `ParentModeQuery`/`ParentModeQueryMessage` (`message` app) — don't conflate the three; see `parent_link_views.py`'s own docstring for the explicit "these are deliberately not the same feature" note. |
| — | **Confirmed gap, re-verified this pass**: no `StudentProgressTests` class exists anywhere in `tests.py` (26 test classes total, none covering `StudentProgressView`). `StudentProgressView` (§1, §4) is currently untested; add a test class here before relying on it in production. |

**Sibling test module**: `test_classroom_chat_bridge.py` (§1, §6b) is a **separate** file, discovered automatically alongside `tests.py` by Django's `test*.py` convention — `python manage.py test liveclass` runs both without extra config. **✅ RESOLVED this pass**: on its fifth upload under this filename, the content is finally the real test suite rather than another copy of `core/classroom_chat_bridge.py`'s source — 11 classes (see §6b for the full list), not the 6 (`CreateClassroomGroupTests`, `JoinAcceptChatSyncTests`, `RemovalChatSyncTests`, `MetadataSyncTests`, `ArchiveOnCloseTests`, `ClassroomStaffChatSyncTests`) this doc had been describing from memory across the prior four mismatched uploads — that 6-class description should now be treated as superseded, not current.
...
5. All File/ImageFields need `MaxFileSizeValidator`; the four plain FileFields (material, assigments attachment, assigments submission, certificate) also need a **safelist** `FileExtensionValidator` — `cover_image` is safe for free via Pillow decoding.
...
22. **A model whose write methods are disabled must raise loudly, not silently no-op.** Task 12 (assigments) and Task 6 (coin models) both retire a local model's write path in favor of a cross-app one — in both cases every retired method is a `RuntimeError` stub naming its replacement, not a quiet `pass`/`return None`. Follow this same shape for any future "the real write path moved to another app" migration: a silent no-op here would look like success to the caller while doing nothing, which is far worse than a loud, traceable crash.
23. **A cross-app bridge's roster/payload shape copied from a sibling bridge is an assumption until the sibling's own source is read, not a fact.** `liveclass/bridge.py`'s `roster` dict shape (`{"user_id": ...}` only, no `roll_number`/`enrollment_no`) was inferred from `campus/bridge.py`'s call site, not from `assigments/bridge.py` itself (not part of this audit) — flagged explicitly in the code rather than padded with guessed placeholder values. Don't quietly "fix" this into certainty; confirm against `assigments/bridge.py` first.
24. ✅ **RESOLVED THIS PASS (PRODUCTION FIX PASS) — three concrete, previously-live bugs, confirmed by direct code read across four uploads, now actually fixed in the code (not just documented):**
    - ~~`ClassSessionViewSet.parent_join` (`views.py`) calls `ParticipantRole.OBSERVER`, which doesn't exist~~ — **fixed**: both call sites (`views.py`, was lines 1879/1886) now use `ParticipantRole.PARENT_OBSERVER`, the enum member `livekit_utils.py`'s `_grants_for_role` was actually built to handle. See §6c.
    - ~~`urls.py` wires 4 `classrooms/<...>/` paths with the `<uuid:classroom_id>` converter, but `Classroom`'s pk is a plain integer~~ — **fixed**: all 4 paths (`create_group/`, `group/`, `participants/<id>/parent-code/`, `parent-queries/`) now use `<int:classroom_id>`, matching every other `classrooms/<...>/` route in the file. See §6b.
    - ~~`ReportCardViewSet._homework_stats()` (`parent_link_views.py`) filters on `assigments.Category.HOMEWORK`, a field/enum `assigments` doesn't have~~ — **crash fixed**: the invalid `category=...` filter is removed, so `POST report-cards/` no longer raises `AttributeError`; every classroom assigments is now counted (there was never a homework-vs-other distinction on this model to filter by). **Still open, flagged in-code rather than guessed at**: this still reads the local, Task-12-frozen `assigments`/`assigmentsSubmission` models directly instead of `bridge.get_assigments_submissions()` (§6d), so it will silently miss any assigments posted after the Task 12 cutover — closing that needs the unified app's row shape confirmed from `assigments/bridge.py`/`assigments/models.py` first (neither was part of this or any prior upload). See §6c.
25. **A module's own docstring (or a whole file's module-level docstring, like `urls.py`'s endpoint reference) can go stale the moment the code beneath it changes, and nothing forces it to be re-synced.** ✅ **RESOLVED THIS PASS**: `urls.py`'s docstring block (the `coin-purchases/initiate/`, `withdrawals/{id}/{cancel,approve,reject,mark-paid}/`, and `assigmentss/`/`submissions/{id}/{grade}` lines) was rewritten to state the actual current behavior — `assigmentsViewSet` (`views.py`) is a plain `list`/`create`-only thin proxy (no `retrieve`/`update`/`destroy`), `assigmentsSubmissionViewSet` is `list`-only (no POST/`{id}`/`grade`), and `CoinWithdrawalViewSet`'s `create`/`cancel`/`approve`/`reject`/`mark_paid` are all 410-Gone stubs pointing at `/api/user-profile/coin-withdrawals/` — each corrected line names the shape that's actually true today and points at §6d/§6e rather than describing pre-Task-6/12 behavior as current. General lesson stands for future changes: trust a ViewSet's own method bodies over a docstring describing them, and re-sync the docstring in the same change that alters the behavior underneath it.
...
27. **(RE-SYNC PASS 1 — no `views.py`/`urls.py`/`tasks.py`/`tests.py` re-supplied yet) Re-checked this doc against a fresh upload of `models.py`, `serializers.py`, `signals.py`, `consumers.py`, `realtime.py`, `livekit_utils.py`, `admin.py`, `chunked_upload_views.py`, `migrate_liveclass_coin_models.py`, plus re-confirmed `bridge.py`/`apps.py`/`classroom_chat_views.py`/`exceptions.py`/`moderation.py`/`notifications.py`/`parent_link_views.py`/`permissions.py`/`routing.py`/`migrate_liveclass_assigmentss_to_unified.py`.** Net result: **no behavioral drift found** in any file that was actually re-uploaded — every model/field/function this pass checked (`Classroom.chat_group_enabled`/`linked_conversation_id`, `StudentReportCard`, `ParticipantRole`, `assigments`'s field set, `ClassroomStaff`, the 6 chat-sync signal receivers, `LiveKitError` as an `APIException`) matches what this file already claimed. What changed is bookkeeping only: (a) §1's line counts were stale for `serializers.py`/`livekit_utils.py`/`parent_link_views.py`/`classroom_chat_views.py`/`realtime.py`/`models.py` — corrected; (b) `serializers.py` actually has **67** classes, not the previously-stated 55 — the 12 uncounted ones are plain output serializers for already-documented endpoints (referrals, earnings, progress, share history), not a new feature — see the note under §1's table; (c) item 24's three live bugs were independently re-verified against the model/util side (not the view/url side, since `views.py`/`urls.py` weren't re-uploaded this pass) and are **still open**. **No `views.py`, `urls.py`, `tasks.py`, `throttles.py`, or `tests.py` content was available this pass** — anything in this file sourced from those (URL paths in §5, task list in §9, test-class lists in §16) is carried forward unverified, same status as before, not newly confirmed.
28. **(RE-SYNC PASS 2 — `views.py`, `urls.py`, `tasks.py`, `tests.py`, `test_classroom_chat_bridge.py`, `throttles.py`, `ws_auth.py` now supplied, closing out most of pass 1's "unverified" list.)** Findings:
    - **Item 24's three live bugs are now DIRECTLY confirmed against `views.py`/`urls.py` themselves** (previously only checked model/util-side) — see the updated bullets under item 24 with exact line numbers. All three remain open, unfixed.
    - **Item 25's stale-docstring claim is now DIRECTLY confirmed** — `assigmentsViewSet` and `CoinWithdrawalViewSet` in `views.py` were read line-by-line and do exactly what §6d/§6e already said (thin proxy / 410-Gone stubs), while `urls.py`'s own module docstring still advertises the old full-CRUD shape. See the updated item 25.
    - **`tasks.py`**: line count (1846) and the "38 `@shared_task`s" count both match this doc exactly, no correction needed. `notify_followers_new_classroom` (line 1790) and `notify_waitlist_promotion` (line 651) both confirmed present and implemented as described in §9/item 16/item 26 — including item 26's flagged, still-unconfirmed `Notification.NotifType.CLASSROOM_CREATED_BY_FOLLOWED` reference (no fallback, `core/models.py` still not part of any upload).
    - **`tests.py`**: grew from ~2610 to 2885 lines, 25→26 test classes — the new one, `ParentTeacherMessageTests`, was previously uncatalogued (§16 table updated). `StudentProgressTests` gap re-confirmed still absent.
    - **`test_classroom_chat_bridge.py`**: STILL the wrong content on this, its **third** upload running — still `core/classroom_chat_bridge.py` source, not the 6 test classes this doc has described from memory since the first mismatch. Downgraded that description to explicitly-unverified in §16.
    - **`throttles.py`/`ws_auth.py`**: both match this doc's existing description exactly (single `ParentJoinIPThrottle` class, scope `session_parent_join_ip`; thin re-export shim respectively) — only their line counts (41, 53) were unconfirmed before, now confirmed.
    - **`urls.py`** grew from ~429 to 510 lines with no new missing-route regression found against what §5 already documents (the growth is consistent with the docstring/comment density seen everywhere else in this codebase, not new endpoints) — line count corrected in §1.
29. **(RE-SYNC PASS 3 — same full file set as pass 2, re-uploaded again: `admin.py`, `bridge.py`, `livekit_utils.py`, `models.py`, `realtime.py`, `serializers.py`, `signals.py`, `tasks.py`, `test_classroom_chat_bridge.py`, `tests.py`, `urls.py`, `views.py`, `moderation.py`, `notifications.py`, `parent_link_views.py`, `permissions.py`, `routing.py`, `throttles.py`.)** No manual-migration step is outstanding anywhere in this app — the earlier `chat_group_enabled`/`linked_conversation_id` "migration still required" caveats (§1, §3, §6b) have been removed; both fields are defined and migrated. Findings this pass, all **bookkeeping corrections, no new behavioral drift**:
    - **`serializers.py`** is actually **1803** lines with **68** classes, not the previously-stated 1772/67 — the 68th, previously-uncatalogued class is `StudentReportCardSerializer` (backs `ReportCardViewSet`, §6c). Corrected in §1.
    - **`admin.py`** is **606** lines (was misstated as 607); the **33** business-model `ModelAdmin`s figure is re-confirmed unchanged (the file also defines one conditional `_AutoUserAdmin` fallback for `User`, already documented separately in §14 and not part of the 33). The `ParentMessageTemplate`/`ParentTeacherMessage` no-admin-registration gap (§14) is re-confirmed **still open**.
    - **`test_classroom_chat_bridge.py`** is **395** lines (was misstated as 396) and, on this **fourth** upload under this filename, is **still** `core/classroom_chat_bridge.py` source rather than the 6 test classes this doc describes from memory — see §1/§16.
    - **Item 24's three live bugs re-confirmed still open, unchanged, same line numbers**: the `<uuid:classroom_id>` path-converter mismatch (`urls.py`), `ReportCardViewSet._homework_stats()` filtering on `assigments.Category.HOMEWORK` (a field/enum `assigments` still doesn't have), and `parent_join()` writing `ParticipantRole.OBSERVER` (`views.py` lines 1879/1886) against a `ParticipantRole` enum (`livekit_utils.py`) that still only defines `HOST`/`CO_HOST`/`STUDENT`/`PARENT_OBSERVER`.
    - **`bridge.py`, `moderation.py`, `notifications.py`, `parent_link_views.py`, `permissions.py`, `routing.py`, `throttles.py`**: re-read in full this pass, byte-for-byte match what §6d/§6c/§10/§11 already documented — no drift.
    - **`tests.py`**: still 2885 lines, still 26 test classes (25 `LiveClassTestBase` subclasses + `ChatModerationScreenMessageTests` on plain `TestCase`), `StudentProgressTests` still absent.
30. ✅ **(PRODUCTION FIX PASS — the three item-24 bugs and the `ParentMessageTemplate`/`ParentTeacherMessage` admin gap actually fixed in code this pass, not just re-confirmed.)** Scope: `urls.py`, `views.py`, `parent_link_views.py`, `admin.py`.
    - **`urls.py`**: the 4 `classrooms/<uuid:classroom_id>/...` paths (`create_group/`, `group/`, `participants/<id>/parent-code/`, `parent-queries/`) now use `<int:classroom_id>`, matching every other `classrooms/<...>/` route in the file — see §6b, item 24. Also rewrote the stale docstring block (`coin-purchases/`, `withdrawals/`, `assigmentss/`/`submissions/`) to state actual current behavior instead of pre-Task-6/12 shapes — see item 25.
    - **`views.py`**: `ClassSessionViewSet.parent_join`'s two `ParticipantRole.OBSERVER` references (generate + response body) are now `ParticipantRole.PARENT_OBSERVER` — confirmed against `livekit_utils.py`'s `_grants_for_role`, which was already built to handle that exact member. See §6c, item 24.
    - **`parent_link_views.py`**: `ReportCardViewSet._homework_stats()`'s crash-causing `category=assigments.Category.HOMEWORK` filter is removed (every classroom assigments is now counted — there was never a homework/other distinction on this model). **Was: "not fully closed" — now ✅ RESOLVED, see §17 item 31**: `_homework_stats()` was updated in a later pass to route through `bridge.get_assigments_submissions()` instead of the legacy local tables. See §6c, item 24.
    - **`admin.py`**: added `ParentMessageTemplateAdmin` (editable — category/title/body_template/requires_session/is_active/display_order, same "staff-tunable copy" reasoning as `moderation.py`'s settings-overridable word list) and `ParentTeacherMessageAdmin` (read-mostly — `resolved_message`/`created_at` in `readonly_fields`, same audit-trail reasoning as `ReferralAdmin`). §14's "still open" gap note and §1's `admin.py` row should be treated as resolved as of this pass — model-admin count is now **35** (33 + these 2), plus the pre-existing conditional `_AutoUserAdmin` fallback (still not counted in that figure, per pass 3's note).
    - **Not touched, no source available to fix safely (as of this pass)**: the `assigments`-bridge migration gap flagged above (since fixed — see §17 item 31), the unconfirmed `Notification.NotifType.CLASSROOM_CREATED_BY_FOLLOWED` reference (item 26), and anything living in `core/classroom_chat_bridge.py`/`settings.py`/`exceptions.py`/`apps.py`/`consumers.py` — none of these were part of this upload.
31. ✅ **(RE-SYNC PASS 4 / TASK 5 FIX PASS — fresh upload of `models.py`, `admin.py`, `chunked_upload_views.py`, `consumers.py`, `livekit_utils.py`, `parent_link_views.py`, `migrate_liveclass_coin_models.py`, plus re-confirmed `bridge.py`, `apps.py`, `classroom_chat_views.py`, `exceptions.py`, `moderation.py`, `notifications.py`, `migrate_liveclass_assigmentss_to_unified.py`, `__init__.py`, and a new migration `0003_studentreportcard_created_by.py`.)** Real behavioral changes this pass, not just bookkeeping:
    - **`models.py`** grew 3502→3539 lines: `StudentReportCard` (§3) gained a real `created_by` `ForeignKey(User, on_delete=SET_NULL, null=True, blank=True, related_name="report_cards_created")` — nullable because the table already had unattributable rows, matching migration `0003_studentreportcard_created_by.py` (`liveclass.0002_initial` → `0003`). This closes the drf-spectacular schema-generation crash `StudentReportCardSerializer` had been silently exposed to (it already listed `created_by` in `fields`/`read_only_fields` before the column existed).
    - **`parent_link_views.py`** grew 318→360 lines (Task 5 fix): `ReportCardViewSet.create()` now passes `created_by=request.user` into its `update_or_create()`, and — **closing the gap items 29/30 explicitly left open** — `_homework_stats()` now routes through `bridge.get_assigments_submissions(classroom)` instead of reading the legacy, Task-12-frozen local `assigments`/`assigmentsSubmission` tables directly. See §6c, §6d, item 24 (now fully resolved) for the updated description.
    - **New gap surfaced (not a regression)**: `StudentReportCard` still has no `ModelAdmin` in `admin.py` — see §14.
    - **`admin.py` (606), `chunked_upload_views.py` (607), `consumers.py` (577), `livekit_utils.py` (472), `migrate_liveclass_coin_models.py` (348), `bridge.py` (267), `apps.py` (33), `classroom_chat_views.py` (86), `exceptions.py` (184), `moderation.py` (137), `notifications.py` (260), `migrate_liveclass_assigmentss_to_unified.py` (258), `__init__.py` (0)**: all re-read this pass, line counts and content match what §1/§6b/§6d/§10/§11 already documented exactly — no drift, now confirmed-exact rather than approximate for `consumers.py`/`chunked_upload_views.py`/`notifications.py` (§1 tildes removed).

---
32. ✅ **(RE-SYNC PASS 5 — fresh upload of `realtime.py`, `serializers.py`, `signals.py`, `tasks.py`, `test_classroom_chat_bridge.py`, `tests.py`, `urls.py`, `views.py`, `permissions.py`, `routing.py`, `throttles.py`.)** Two genuine, related findings — both close out standing gaps rather than open new ones:
    - **✅ TASK 20 dead-code deletion finally happened**: `PARENT_JOIN_TOKEN_SALT`/`generate_parent_join_token()` (`views.py`) and `ParentJoinSerializer` (`serializers.py`) — flagged since the parent-join consolidation as unused-but-not-yet-deleted (§6c) — are now actually removed from the codebase, each replaced by an explanatory comment citing a confirmed project-wide grep with no remaining callers. `views.py` 6508→6498 lines, `serializers.py` 1803→1759 lines / 68→67 classes. See §1, §6c (updated), §18 checklist (updated).
    - **✅ `test_classroom_chat_bridge.py` content mismatch finally resolved, on its fifth upload under this filename**: this is now genuinely `core/classroom_chat_bridge.py`'s test suite (395→492 lines) — 11 `TestCase` classes, mock/`SimpleNamespace`-based rather than real-model-based. The 6-class description this doc had carried forward from memory across four stale uploads is superseded — see §1, §6b, §16 (all updated).
    - **`urls.py`** grew 510→525 lines — comment expansion from the already-documented TASK 15 path-converter fix (item 30), no new routes; the fix itself is re-confirmed present (`<int:classroom_id>` on all four `classrooms/<...>/` paths). **No regression.**
    - **`realtime.py` (558), `signals.py` (620, confirmed exact — was `~620`), `tasks.py` (1846, 38 `@shared_task`s re-confirmed), `tests.py` (2885, 26 classes re-confirmed), `permissions.py` (additive snippet, not a full file — unchanged), `routing.py` (30), `throttles.py` (41)**: all re-read this pass, no drift from what §1/§9/§16 already documented.
    - **Item 26's open question re-checked, still open**: `tasks.py` line 1837 still references `Notification.NotifType.CLASSROOM_CREATED_BY_FOLLOWED` with no fallback; `core/models.py` still hasn't been part of any upload, so this remains unconfirmed.
...
- **Parent-facing work** → check §6c first — parent auth now runs through exactly one mechanism (`ParentAccessCode`/`ParentToken`, resolved via `core.classroom_chat_bridge.resolve_parent_from_token()`), used by `ClassroomParentCodeGenerateView`, `ReportCardViewSet`, `ClassroomParentQueryListView`, `ParentQueryReplyView`, and `ClassSessionViewSet.parent_join` alike. The old signed-token path (`PARENT_JOIN_TOKEN_SALT`/`generate_parent_join_token`/`ParentJoinSerializer`) has been deleted (TASK 20) — don't resurrect it, see §6c.
- **assigmentss** → check §6d first. New assigmentss never touch `liveclass.assigments`/`assigmentsSubmission` directly — go through `bridge.create_assigments()`/`get_assigments_submissions()`, which delegate to `assigments.bridge.*`. The local models are read-only history now; the only sanctioned exception to "liveclass never imports `assigments.models` outside `bridge.py`" is the one-off `migrate_liveclass_assigmentss_to_unified` command. If `assigments/bridge.py`'s real `create_context_assigments()` signature requires `roll_number`/`enrollment_no` keys unconditionally, `bridge.py`'s roster dict needs a one-line fix — this is a flagged, unconfirmed assumption, not settled fact (see §6d).
