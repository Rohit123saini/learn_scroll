...
> **Reconciliation pass (newest of all — this update):** Saare source files firse code ke against check kiye gaye. **2 real gaps mile, plus ek documentation-hygiene fix:**
...
| `tests.py` | `Notification`/`NotificationPreference`/`create_notification`/viewset/`SearchView` ke **37** tests total — `SearchViewTests` (12 tests, naye is pass me confirm hue) `SearchView` ke liye ab test coverage deta hai, see §9 item 14 |
...
- **✅ TEST COVERAGE — `tests.py::SearchViewTests`, 12 tests.** `q`/`sources` parsing, 400-on-missing/empty/whitespace/below-min-length query, success path (`search_everything()` result pass-through, no `sources`→every source), `assigments`/`testseries` scoped-queryset SQL-shape regressions (staff vs non-staff, four-way OR for testseries), aur `message`/`campus_notice` ke unregistered-but-silent-empty behaviour ka lock-in test. `search_everything()` khud mock kiya gaya hai zyada tests me (real `assigments`/`testseries` model schema is doc ke kisi upload ka hissa nahi raha) — see class docstring.
...
15. **Still open:** `SearchView` sirf `assigments`/`testseries` ke liye scoped queryset banata hai — `message`/`campus_notice` (`Notice`) `search.py`'s `SOURCES` me registered hain (§6.2) lekin `SearchView.get()` unke liye koi queryset nahi banata, isliye `?sources=message`/`?sources=campus_notice` aaj silently khaali result dete hain. Ab is behaviour ke liye ek regression test bhi hai (`test_unregistered_sources_return_empty_not_error`), jo current-incomplete-state ko lock karta hai — inke liye scoped-queryset builder add karna hai (§7 me flag kiya) jab ye tasked ho.
