...
> **Reconciliation pass (newest of all — this update):** Saare source files firse code ke against check kiye gaye. **2 real gaps mile** — dono kisi purane khule open-item ko (poora ya partially) resolve karte hain:
> - **Task 13 — `SearchView` ab `message`/`campus_notice` ke liye bhi scoped queryset banata hai.** Pichli pass ka §9 item 15 ("`message`/`campus_notice` abhi bhi unscoped, `?sources=message` silently khaali deta hai") ab **RESOLVED** hai. `core/views.py::SearchView.get()` me do naye blocks hain: `message` — `ConversationViewSet.search_all()` (`message/views.py`) ko exactly mirror karta hai (current membership `left_at__isnull=True`, non-expired disappearing messages, `deleted_for_everyone`/`deleted_for_users`/`is_scheduled` sab exclude); `campus_notice` — `NoticeViewSet.get_queryset()` (`campus/views.py`) ko mirror karta hai, `campus.views.get_my_campus_ids(user)` seedha reuse karke. Dono ab `assigments`/`testseries` ke saath `scoped_querysets` dict me hain — total 4/4 sources ab wired. `tests.py`'s `SearchViewTests` ab **12 nahi, 15** tests hai (naye: `test_success_path_scoped_querysets_include_all_wired_sources`, `test_message_and_campus_notice_sources_are_wired_not_skipped`, `test_message_scoping_matches_conversation_search_all`, `test_campus_notice_scoping_filters_by_my_campus_ids`) — `tests.py` ka total ab **37 nahi, 40**. `test_unregistered_sources_return_empty_not_error` ab genuinely-unregistered `post` source ke liye hai (pehle ye `message`/`campus_notice` ke "abhi khule" incomplete-state ko lock karta tha — ab uska scope narrow ho gaya hai kyunki wo dono ab registered+scoped hain). §2, §6.2, §7, §9 update kiye.
> - **Task 14 — `classroom_chat_bridge.py` me naya `generate_campus_session_token(room_name, user)` function (#13) add hua**, jo pichli pass ke §9 item 16 ("`provision_video_room()` sirf room-name banata hai, per-participant LiveKit token mint karne wala function missing tha") ko **partially resolve** karta hai. Ye function `message.livekit_utils.generate_livekit_token()` ko ek participant ke liye call karta hai (room-name + user-id + display-name), same "campus kabhi seedha `message` import nahi karta" bridge-pattern follow karte hue. Module ka apna docstring ab khud ko **14 public entry points** bolta hai (pehle 12) — `resolve_parent_from_token()` isi wajah se **#12 se #14 pe** dobara renumber ho gaya. **Abhi bhi khula:** `campus/views.py` ka naya `join` action jo isko actually call kare, wo file is pass me upload nahi hui — module docstring khud kehta hai `campus/views.py` "iss pass available nahi tha", isliye end-to-end wiring (bridge-function se leke real endpoint tak) abhi bhi unconfirmed hai, sirf bridge-side gap band hui hai. §2, §6, §9 update kiye.
>
> **Reconciliation pass (older, superseded by above for its own 2 items — kept for history):** Saare source files firse code ke against check kiye gaye. **2 real gaps mile, plus ek documentation-hygiene fix:**
> - **`NotifType` enum ab 55 nahi, 56 values hai.** `models.py`'s apna docstring (item 6, "G-6, this pass") confirm karta hai ki ek naya value — `PARENT_DEVICE_PENDING` (`"parent_device_pending"`, 22 chars) — is pass me add hua, Parent Mode ke `ParentToken` ke `PENDING` status ke liye (student ko pehle koi notification nahi milti thi jab parent device access maangta). Ye `MESSAGE_APP_TYPES` me bhi add hua hai (ab 4 values, pehle 3). §3 aur §2 ki file-table dono jagah count fix kiya. Model ka apna docstring khud kehta hai ki is value ke liye state-only `AlterField` migration **abhi generate nahi hui** ("that migration file wasn't part of this pass's upload") — isliye ye ek naya open item hai (§9), koi maujooda migration file nahi.
> - **`tests.py` me ab `SearchViewTests` class hai — 12 naye tests, jo pichli pass ke open items #14/#15 ko seedha address karte hain.** File ka total ab **25 nahi, 37** tests hai. Item #14 (SearchView ke liye koi test nahi) ab **resolved** — success path, `q`/`sources` parsing, 400-on-short-query, aur `assigments`/`testseries` scoping regression (SQL-shape assertions via `str(queryset.query)`) sab covered hain. Item #15 (message/campus_notice abhi bhi unscoped) **khula hi hai** — lekin ab `test_unregistered_sources_return_empty_not_error` isi current (incomplete) behaviour ko ek regression test se lock kar deta hai, taaki koi silently isse aur na todein. §2, §6.2, §7, §9 update kiye.
> - **Documentation-hygiene fix — migration file references jo kabhi verify nahi ho paayi (migrations/ folder kisi bhi pass me upload nahi hui):** is doc me kai jagah specific migration filenames (`0001_move_notification_models.py`, `0002_widen_message_and_add_data.py`, `0004_alter_notification_notif_type.py`) ko fact ki tarah likha gaya tha, jabki koi actual migration file kabhi is doc ki source list ka hissa nahi rahi — sirf model/comment docstrings ke references the. In sab jagah language ko "migration chahiye/expected hai (convention ke mutabik), file khud verify nahi hui" me badal diya gaya hai — koi bhi "manual migration" jaisa pending-task framing hata diya gaya hai jab wo asal me kisi verified file se backed nahi tha. Code jo bhi kehta hai (choices-only change = koi DB-level migration nahi chahiye) wahi is doc ka ab single source of truth hai.
...
| `management/commands/check_config_drift.py` | 4 automated config-drift checks (throttle-scope, celery-beat, admin-registration, urls-wiring), `settings.CONFIG_DRIFT_APPS` (default `["user_profile", "core"]`) par scoped. **§11** poora document karta hai |
| `tests.py` | `Notification`/`NotificationPreference`/`create_notification`/viewset/`SearchView` ke **40** tests total — `SearchViewTests` (**15** tests, Task 13 ke 4 naye message/campus_notice-scoping tests including) `SearchView` ke liye poora test coverage deta hai, see §9 items 14/15 |
...
# --- Task 44 — message app (3) + G-6 (this pass, 1) = 4 ---
CHAT_MESSAGE, MENTION, INCOMING_CALL, PARENT_DEVICE_PENDING
...
```
**Total 56 values** (28 + 4 + 2 + 9 + 8 + 5). Message-app block ab 4 hai, 3 nahi — `PARENT_DEVICE_PENDING` (G-6, is pass) add hua: Parent Mode ke `ParentToken` ke `status=PENDING` landing pe student ko pehle koi notification nahi milti thi (`ParentPendingRequestsView` polling hi ek tarika thi discover karne ka).
...
- Har naya block choices-only addition hai (koi schema/DB-level migration nahi chahiye) — lekin ab is codebase ka convention hai ki phir bhi ek state-only `AlterField` migration record ki jaati hai taaki `makemigrations --check` CI me drift na pakde. Naye `PARENT_DEVICE_PENDING` value ke liye ye migration **abhi generate nahi hui** — model ka apna docstring (G-6) explicitly kehta hai "that migration file wasn't part of this pass's upload, so it still needs to be generated against this change before it lands" (see §2, §9 naya open item). Koi bhi specific migration filename is doc me ab claim nahi ki jaati jab tak wo file khud verify na ho jaye.
...
|---|---|---|
| `MESSAGE_APP_TYPES` (task 46) | `CHAT_MESSAGE`, `MENTION`, `INCOMING_CALL`, `PARENT_DEVICE_PENDING` (G-6, is pass add hua) | `views.py`/`serializers.py` dono yahi ek jagah se import karte hain — naya message-app type add karte waqt sirf yahi ek jagah update karni hai. |
...
| 11 | `provision_video_room(live_session, actor)` | `campus.CampusLiveSession` schedule/create hone pe, `campus/bridge.py` se | LiveKit room-identifier banata hai aur plain string return karta hai (`f"campus_live_session_{live_session.id}"`), jo caller `CampusLiveSession.room_id` (`CharField`) me store karta hai. **Yahan koi LiveKit JWT mint NAHI hoti** — `message/views.py` ke real call-sites (`CallInitiateView`/`StudyRoomJoinView`) confirm karte hain ki `generate_livekit_token(room_name, user_id, user_name)` har participant ke liye JOIN-TIME pe, per-participant call hoti hai, ek baar upfront nahi — ek token short-lived aur viewer-specific hota hai, abhi banaya to student actual join karne se pehle hi stale ho jaata. `actor` signature me accepted hai (campus/bridge.py isi se call karta hai) par unused — permission check yahan nahi hai, `CampusLiveSessionViewSet.perform_create` se already gate maana gaya hai. Actual per-participant token-minting function ab **#13** hai (neeche) — jab tha nahi. |
...
### 13. `generate_campus_session_token(room_name, user)` — **NEW is pass, Task 14 — is doc me pehle bilkul mention nahi tha**

Function #11 (`provision_video_room`) ka apna docstring hamesha se ye exact gap flag karta tha: wo sirf room ka **naam** deta hai, koi token nahi — ye function wahi khula gap band karta hai, "join this live session" ke liye har participant ke liye alag-alag, join-time pe call hone wala.

- **Signature:** `generate_campus_session_token(room_name: str, user) -> str`.
- `room_name` — `provision_video_room()` ne jo room-id already generate karke `CampusLiveSession.room_id` pe save kiya hota hai, wahi caller yahan pass karta hai; ye function khud koi model I/O nahi karta, session ko lookup nahi karta — same "sirf naam/token deta hai, persistence caller ki zimmedari hai" posture jo `provision_video_room()` already follow karta hai.
- `user` — jo participant actually join kar raha hai (zaroori nahi ki session schedule karne wala teacher ho). **Authorization check yahan NAHI hai** — caller (campus/views.py ka `join` action) ki zimmedari hai ye confirm karna ki `user` is room me allowed hai (enrolled student, assigned subject-teacher, class-teacher, ya campus admin) *pehle*; token mint karna khud ek authorization check nahi hai.
- `_display_name_for_token(user)` (naya internal helper) se display-name resolve karta hai — `get_full_name()` (agar exist kare) → `username` → `email` → `str(user.id)`, is order me degrade karta hai, kyunki ye module kabhi assume nahi karta ki real `User` model ka exact shape kya hai (custom user models vary karte hain).
- `message.livekit_utils.generate_livekit_token(room_name=, user_id=, user_name=)` ko local-import karke seedha call karta hai — same signature jo `message/views.py`'s `CallInitiateView`/`StudyRoomJoinView` already use karte hain (function #11 ke docstring me confirmed).
- `LIVEKIT_API_KEY`/`LIVEKIT_API_SECRET` configured na ho to `RuntimeError` raise karta hai (`generate_livekit_token()` ka apna lazy check) — **yahan catch NAHI hoti**, deliberately, taaki caller (`campus/views.py`'s `join` action) ise ek clean `503` me convert kar sake raw `500` ke bajaye — same "RuntimeError surface hone do" convention jo `create_classroom_group()`/`create_section_group()` apne explicit-action (non-signal) callers ke liye already follow karte hain.

**Abhi bhi khula (§9 item 16, is pass me updated, poora resolve nahi hua):** `campus/views.py` ka wo `join` action jo is function ko actually call karega, wo file is pass me bhi upload nahi hui — module docstring khud confirm karta hai. Isliye room-naming (#11) se lekar per-participant-token-minting (#13, ab exist karta hai) tak ka **bridge-side** path ab complete hai, lekin end-to-end "actual endpoint isse call karta hai" wiring abhi bhi unconfirmed hai.
...
**✅ RESOLVED — `SearchView`/`search/` URL wired hai.** `core/views.py::SearchView` (plain `APIView`, Task 18) is file ke `search_everything()` ko `core/urls.py`'s `path("search/", SearchView.as_view())` ke through expose karta hai — poori detail §7 me. Scoped-queryset-building responsibility (golden rule ke mutabik) `SearchView` khud nibhata hai, `search.py` nahi — is file me is wajah se koi change nahi karna pada. **✅ Test coverage** — `tests.py::SearchViewTests` (**15 tests**, §9 items 14/15 resolved). **✅ Ab saare 4 registered sources scoped hain** — `assigments`/`testseries` (Task 18) ke saath `message`/`campus_notice` (Task 13, is pass) bhi. `post`/`class_material` sources upar wali table ke mutabik abhi bhi stub hain (koi model upload na hone ki wajah se) — sirf ye hissa khula hai.
...
- **Is view ki asli responsibility** — golden rule (§6.2) ke mutabik — HAR source ke liye ek already-permission-scoped queryset khud banana hai, `core.search` ko kabhi seedha model query nahi karne dena:
  ...
  - `message` **[Task 13, NEW is pass]** — `message.models.Message` ko `ConversationViewSet.search_all()` (`message/views.py`) jaisa hi scope karta hai: `conversation__memberships__user=user` aur `conversation__memberships__left_at__isnull=True` (current membership only), plus `expires_at__isnull=True` YA `expires_at__gt=now()` (non-expired disappearing messages), aur `deleted_for_everyone=True` / `deleted_for_users=user` / `is_scheduled=True` teeno exclude. `BlockedUser` deliberately filter NAHI hota — us model ke apne docstring ke mutabik wo sirf ek websocket-delivery-time check hai, stored-message visibility rule nahi, isliye yahan add karna ek nayi access-rule invent karna hota (view ke apne intro paragraph ka golden-rule violation).
  - `campus_notice` **[Task 13, NEW is pass]** — `campus.models.Notice` ko `NoticeViewSet.get_queryset()` (`campus/views.py`) jaisa hi scope karta hai: `campus_id__in=get_my_campus_ids(user)`, `campus.views.get_my_campus_ids` ko seedha reuse karke (staff/student/parent-link union khud re-derive nahi karta — us function ka apna docstring kehta hai ye centralized isliye hai taaki har naya membership route add hone par sab jagah sync rahe). `NoticeViewSet`'s apna `?campus=` query-param narrowing is view ka equivalent scoping-param nahi hai — `SearchView` uska mirror nahi karta.
- Sab local imports (`from assigments.models import ...`, `from testseries.models import ...`, `from message.models import Message`, `from campus.models import StudentEnrollment, Notice`, `from campus.views import get_my_campus_ids`) function-body ke andar hain, module-level nahi — same lazy-import posture jo poore project me circular-import se bachne ke liye use hoti hai.
...
- **✅ TEST COVERAGE — `tests.py::SearchViewTests`, 15 tests.** `q`/`sources` parsing, 400-on-missing/empty/whitespace/below-min-length query, success path (`search_everything()` result pass-through, no `sources`→every source, aur **[NEW]** `scoped_querysets` me sab 4 wired sources present hone ka check), `assigments`/`testseries` scoped-queryset SQL-shape regressions (staff vs non-staff, four-way OR for testseries), **[NEW, Task 13]** `message`/`campus_notice` ke apne SQL-shape regressions (`memberships`/`left_at`/`expires_at`/`deleted_for_everyone`/`deleted_for_users`/`is_scheduled` sab `message` queryset ki SQL me hone chahiye; `campus_id` `campus_notice` ki SQL me), aur ek genuinely-unregistered source (`post`) ke liye clean-empty-not-error lock-in test. `search_everything()` khud mock kiya gaya hai zyada tests me (real `assigments`/`testseries`/`message`/`campus_notice` model schema is doc ke kisi upload ka hissa nahi raha) — see class docstring.
...
core.views.SearchView  ──uses──▶  core.search.search_everything   [Task 18, WIRED]
                            ...
                            ├──local-import──▶ campus.models (StudentEnrollment, Notice), campus.views (get_my_campus_ids)   [Notice/get_my_campus_ids: NEW, Task 13]
                            ├──local-import──▶ testseries.models (TestSeries)
                            └──local-import──▶ message.models (Message)   [NEW, Task 13]

campus.views (join action, NOT uploaded this pass)  ──(unconfirmed)──▶  core.classroom_chat_bridge.generate_campus_session_token   [Task 14, bridge-side ready, caller unconfirmed]
...
18. **NEW open item (this pass):** naye `PARENT_DEVICE_PENDING` `NotifType` value (§3) ke liye state-only `AlterField` migration **abhi generate nahi hui** — model ka apna docstring (G-6) ye explicitly flag karta hai. Ye ek routine `makemigrations` run hai (koi hand-written/"manual" migration nahi, no DB-level change), lekin jab tak generate na ho, `makemigrations --check` CI me drift pakdega.
